...
def init_state() -> None:
    defaults = {
        ...
        "nuke_confirm": False,
        "last_llm_error": None,   # most recent live LLM error (None == ok)
    }
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        elif st.session_state.get("last_llm_error"):
            st.error(f"Last live call failed:\n`{st.session_state['last_llm_error'][:200]}`")
...
def render_pattern_section() -> None:
    ...
    components.render_pattern_card(pattern)
...
# LLM error surfacing
# --------------------------------------------------------------------------- #
def render_llm_error() -> None:
    """Show the real API error when a live call failed (so it isn't hidden)."""
    err = st.session_state.get("last_llm_error")
    if not err:
        return
    # Only surface if we actually have a live client (otherwise it's just offline).
    if not llm.is_live():
        return
    st.error(
        f"**Live Gemma call failed** — Evolve fell back to a synthetic reply. "
        f"The underlying error was:\n\n`{err}`\n\n"
        "Common causes: the model id isn't enabled for your key "
        "(`EVOLVE_MODEL`), the key lacks access, a regional restriction, or a "
        "rate limit. The routing/draft reply still works; fix the API access "
        "for real Gemma output."
    )
...
def handle_input(user_input: str) -> None:
    ...
    # Capture any live-call failure so it can be surfaced (not silently mocked).
    st.session_state.last_llm_error = llm.get_last_error()
...
def main() -> None:
    ...
    # Surface any live-call failure (does nothing when offline).
    render_llm_error()
