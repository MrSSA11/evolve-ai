...
- Presentational HTML blocks (header, status banner, memory timeline, knowledge
  clusters, cited sources) are rendered via ``streamlit.components.v1.html``.
  This injects a real iframe and **bypasses Streamlit's markdown HTML
  sanitizer**, so the markup always renders correctly (instead of leaking as
  raw ``<div>`` text, which can happen with ``st.markdown(unsafe_allow_html=True)``
...
def render_html(inner_html: str, height: int) -> None:
    ...
    Falls back to ``st.markdown`` only if the component renderer is unavailable.
    """
...
def render_analytics_cards(stats: dict, session_state=None) -> None:
    ...
    ``flex: 1 1 240px`` matrix, but without fragile CSS overrides on Streamlit's
    viewport divs and without relying on ``unsafe_allow_html`` rendering.
    """
    current = "Personal"
    friction = 0.0
    switches = 0
...
def render_memory_sources(recall_ctx: dict | None, citations: list[int] | None) -> None:
    ...
    if not recall_ctx:
        st.markdown(
            '<span style="color:#8A99AD;font-size:0.85rem;">No active recall yet — '
            "send a message to pull memory frames.</span>",
    ...
    if not frames:
        st.markdown(
            '<span style="color:#8A99AD;font-size:0.85rem;">No frames recalled for the last query.</span>',
            unsafe_allow_html=True,
        )
        return
...
def render_memory_timeline(frames: list[dict], limit: int = 14) -> None:
    ...
    if not frames:
        st.markdown(
            '<span style="color:#8A99AD;font-size:0.85rem;">No memory frames yet.</span>',
            unsafe_allow_html=True,
        )
        return
...
def render_knowledge_clusters(clusters: list[dict]) -> None:
    ...
    if not clusters:
        st.markdown(
            '<span style="color:#8A99AD;font-size:0.85rem;">No clusters yet — '
            "run <b>Improve</b> in the sidebar to synthesize.</span>",
            unsafe_allow_html=True,
        )
        return
...
def render_code_workspace(blocks: list[dict]) -> None:
    ...
    if not blocks:
        st.markdown(
            '<span style="color:#8A99AD;font-size:0.85rem;">'
            "No code blocks in the last response.</span>",
            unsafe_allow_html=True,
        )
        return
    ...
    if files:
        st.markdown(
            '<span style="color:#8A99AD;font-size:0.74rem;">Workspace files:</span>',
            unsafe_allow_html=True,
        )
        for fl in files[-12:][::-1]:
            st.markdown(
                f'<span style="color:#8A99AD;font-size:0.74rem;">'
                f"• {fl['name']} <span style='opacity:0.6;'>({fl['size']} B)</span></span>",
                unsafe_allow_html=True,
            )
